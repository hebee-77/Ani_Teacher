...
export default function Chat({
  ...
  // Clear input when messages are reset (new chat)
  useEffect(() => {
    if (messages.length === 0) setInput('');
  }, [messages]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errMsg = 'Sorry, I encountered an error connecting to my brain.';
      setMessages(prev => {
        const updated = [...prev];
          ...
          <div className="flex flex-col gap-6 w-full">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} w-full`}>
                <div
                  className={`output-content ${msg.role === 'user' ? 'bg-surface2 border border-border px-4 py-3 rounded-2xl rounded-tr-sm max-w-[80%]' : 'w-full'}`}
                  style={{ animation: 'none' }}
                >
                  {msg.role === 'ai' && msg.content === '' && isStreaming ? (
                    <p style={{ color: 'var(--text3)', fontSize: '13px' }}>✦ Generating explanation...</p>
                  ) : msg.role === 'user' ? (
                    <div style={{ fontSize: '13.5px', color: 'var(--text2)', whiteSpace: 'pre-wrap', fontFamily: 'var(--font)' }}>
                      {msg.content}
                    </div>
                  ) : (
                    <>
